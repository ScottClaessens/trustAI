# function to extract participant means in study 4
extract_study4_pid_means <- function(study4_fit2) {
  # get fitted values
  newdata <- 
    expand_grid(
      Trait = c("trust", "reliable", "good-intentions"),
      PID = sort(unique(study4_fit2$data$PID))
    )
  f <-
    fitted(
      object = study4_fit2,
